...
The contribution is a design pattern, systems abstraction, reference architecture, and evaluation agenda, not empirical validation. We define the stewardship pattern, the inner stewardship harness, and executable context patterns; distinguish agent interoperability from maintained executable state; outline an MCP-accessible inner-harness interface; and propose criteria for testing cross-agent drift and maintained-state quality.
\end{abstract}
...
The contribution is architectural: it defines the maintained executable state that agent loops consume and update, rather than claiming empirical performance gains. The claims are deliberately bounded: the stewardship pattern does not replace A2A or communication, eliminate drift, guarantee better final answers, or apply to every task.
...
The failure modes above are not experimental results in this paper. They are motivating examples that the proposed architecture is intended to make visible and that future evaluations should measure directly.
...
\bottomrule
\end{tabular}
\caption{Motivating failure modes that future evaluations should measure directly.}
...
This minimum version excludes general agent-to-agent communication, broad social negotiation, full workflow orchestration, automatic proof that all dependencies were found, and automatic acceptance of agent-proposed context-pattern changes. Even a small inner harness should make current state explicit, make context materialization executable, record supersession, and support integration of task discoveries into maintained state.
...
\item \textbf{Execution and materialization:} run a selected block's steps or graph region with runtime inputs and slot bindings; materialize the context bundle, intermediate artifacts, final artifacts, and provenance implied by that run.
...
We proposed the stewardship pattern for multi-agent AI systems with long-lived shared state: agent harnesses use an MCP-accessible inner stewardship harness to consume, execute, and improve shared executable structure. A2A lets agents find and communicate with other agents; MCP lets outer loops access the inner harness; the inner harness maintains artifacts, context patterns, authority, supersession, invalidation, downstream-impact analysis, and integration of task discoveries into maintained state.
