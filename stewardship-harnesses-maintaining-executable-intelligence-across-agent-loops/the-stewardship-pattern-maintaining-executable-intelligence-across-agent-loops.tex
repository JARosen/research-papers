...
Maintaining Executable Intelligence\\
Across Agent Loops}
...
\item We provide a reference architecture, MCP-accessible inner-harness interface, lifecycle model, limitations, and evaluation protocol for measuring cross-agent drift and stewardship quality.
\end{itemize}
...
Solution & Couple agent harnesses to one inner stewardship harness through MCP; select and execute maintained structure during work; route observations, artifact changes, downstream-impact checks, and review through the lifecycle in Section~\ref{sec:lifecycle}. \\
...
\bottomrule
\end{tabular}
\caption{The stewardship pattern as a compact systems-architecture specification.}
...
\section{Operational Example: Company Intelligence and Strategy Analysis}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.05\linewidth}
                ...
                >{\raggedright\arraybackslash}p{0.33\linewidth}@{}}
\toprule
\textbf{No.} & \textbf{Specified operation} & \textbf{Maintained-state update} & \textbf{Required semantic effect} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Specified state-transition semantics for the company-intelligence scenario. This is a toy semantic trace for the architecture, not observed data or benchmark results.}
...
\toprule
\textbf{Failure mode} & \textbf{Typical behavior in loop-centric systems} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Illustrative failure modes measured by the maintained-state evaluation protocol.}
...
\end{itemize}
The tuple is a compact index for the state model rather than a separate implementation requirement; the following sections expand these components into record semantics, context-pattern execution, interface operations, and lifecycle transitions.
...
\item a run execution planner that identifies artifacts to reuse, rebuild, supersede, mark stale, produce, or route for review,
...
\subsection{Improving Executable Context Patterns}
...
\item \textbf{Compilation and versioning:} compile raw structure into an immutable executable version; resolve the current compiled version; preserve prior versions for audit, rollback, and downstream-impact analysis.
\item \textbf{Execution and materialization:} plan and run a selected block's steps or graph region with runtime inputs and slot bindings; materialize the context bundle, intermediate artifacts, final artifacts, and provenance implied by that run.
...
The interface has one central design requirement: a loop can discover or select an existing executable graph region, run it with task-specific inputs and slots, consume the resulting context and artifacts, and integrate observations or proposed edits into maintained structure through schema-described operations. These semantics may be exposed as high-level materialization operations or emerge from lower-level project, block, version, run, artifact, and review operations.
...
\node[lifestep, below=18mm of obs] (affect) {downstream-impact\\analysis};
...
\end{tikzpicture}
\caption{Stewardship is integrated into task execution. The outer loop selects and runs inner-harness structure early, consumes generated artifacts while doing the work, and routes observations through downstream-impact analysis, review, and integration steps before current state is accepted.}
...
\section{Evaluation Protocol for Maintained-State Quality}
...
\begin{itemize}[leftmargin=*]
\item \textbf{Context-pattern quality:} reuse rate, improvement acceptance rate, context precision and recall, duplicate procedure creation, and future-agent lift from prior context-pattern improvements.
\item \textbf{Artifact-state quality:} stale artifact rate, authority conflicts, stale or superseded artifact consumption, execution-plan accuracy, invalidation correctness, propagation completeness, and cross-agent consistency.
...
\item \textbf{Wiki and repository comparison:} stale page or file consumption, rediscovered procedures despite shared content, updates that become executable context-pattern changes, time to assess downstream impact, file changes without dependency metadata, and future-loop use of the correct current artifact.
\end{itemize}
...
\subsection{Benchmark Instance Structure}

A benchmark instance contains:
\begin{itemize}[leftmargin=*]
\item an initial compiled project version with blocks, executable steps, context patterns, artifact contracts, slots, and maintained artifacts,
\item a sequence of task requests issued to independent agent harnesses,
\item one or more perturbations to criteria, source state, or authority policy,
\item gold current artifacts for each role and scope after the perturbation,
\item gold downstream-impact sets for changed artifacts or context patterns,
\item expected stewardship actions such as artifact supersession, stale marking, context-pattern update, review routing, or no-update-needed justification.
\end{itemize}

Continuing the lifecycle trace in Table~\ref{tab:lifecycle-trace}, a company-intelligence benchmark instance may start with:
\begin{itemize}[leftmargin=*]
\item \texttt{company\_profile:X:v2}
\item \texttt{technical\_novelty:X:v1}
\item \texttt{risk\_criteria:infra:v1}
\item \texttt{market\_map:infra:v3}
\item \texttt{competitor\_risk\_pattern:v1}
\item \texttt{risk\_assessment:X:v1}
\end{itemize}

The perturbation states: ``Infrastructure risk criteria must distinguish research-heavy companies from services-heavy companies.'' Gold stewardship behavior follows the transition in Table~\ref{tab:lifecycle-trace}: supersede the criteria, update the context pattern, mark dependent risk and customer-implication artifacts stale, preserve unrelated artifacts such as founder background, and record review or no-update-needed state where applicable. This is a benchmark specification, not measured performance.

\subsection{Benchmark Construction Guidelines}

Benchmark instances should vary the number of maintained artifacts, graph depth, fan-out, number of independent agent harnesses, and perturbation frequency. Small instances can contain a few dozen artifacts and one perturbation; larger instances can contain hundreds of artifacts, multiple context patterns, and several interleaved criteria or source changes. Gold current-state sets can be constructed by authoring the dependency graph and authority scopes directly, then applying perturbations whose affected artifacts are known from the graph. Gold downstream-impact sets should include both positive cases, where recomputation or review is required, and negative cases, where unrelated artifacts must remain current. The benchmark should also specify which agents receive which task requests, so that independent loops, shared wiki or repository baselines, memory baselines, A2A-only delegation, and inner-harness implementations are tested against the same task sequence.
...
\begin{equation}
\mathrm{CurrentAccuracy} = \frac{|C_R|}{|C_G|}
\end{equation}
...
\begin{equation}
\mathrm{ImpactPrecision} = \frac{|R \cap G|}{|R|}
\qquad
\mathrm{ImpactRecall} = \frac{|R \cap G|}{|G|}
\end{equation}
...
\begin{equation}
\mathrm{UpdatePrecision} = \frac{|U \cap G_U|}{|U|}
\qquad
\mathrm{UpdateRecall} = \frac{|U \cap G_U|}{|G_U|}
\end{equation}
...
\item \textbf{Bad structure and context-pattern drift.} A flawed context pattern, incorrect authority decision, or overfit procedure may make future loops consistently wrong.
\item \textbf{False invalidations and missed dependencies.} Overbroad downstream-impact analysis can create unnecessary recomputation and review; omitted dependencies can leave stale artifacts active.
\item \textbf{Stewardship noise and governance bottlenecks.} Many agents may propose edits, context-pattern diffs, stale markings, and invalidations. Without thresholds, deduplication, batching, ownership metadata, and review queues, stewardship activity can become noise or concentrate into slow approval queues.
