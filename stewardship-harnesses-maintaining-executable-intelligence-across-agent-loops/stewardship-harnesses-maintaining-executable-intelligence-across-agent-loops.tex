...
\bottomrule
\end{tabular}
\caption{Adjacent shared-state mechanisms can support a stewardship harness, but do not by themselves define the agent-facing stewardship protocol proposed here unless extended with additional semantics.}
...
MCP & Schema-described access to harness operations, resources, runs, artifacts, and context. & Maintained-state semantics unless backed by a harness or equivalent application layer. \\
Shared stewardship harness & Current state, durable artifacts, executable context patterns, compiled versions, runs, supersession, and propagation. & General communication, social negotiation, or all agent-to-agent coordination as its primary role. \\
\bottomrule
\end{tabular}
