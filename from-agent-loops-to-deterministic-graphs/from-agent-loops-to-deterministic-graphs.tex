...
We compare execution-lineage replay against loop-centric update baselines on two controlled update tasks in a fast-demo telehealth setting. In an unrelated-branch update, DAG replay preserved the final memo exactly in all runs, with zero churn and zero unrelated-branch contamination, while loop baselines rewrote the memo and frequently imported unrelated context. In an intermediate-artifact edit, all systems reflected the new constraint in the final memo, but only DAG replay achieved perfect upstream preservation, downstream propagation, unaffected-artifact preservation, and cross-artifact consistency.
...
\item \textbf{RQ3: Maintained-state quality vs.\ final deliverable quality.} Do loop-centric baselines and execution-lineage replay differ more on maintained-state metrics than on final memo correctness?
\item \textbf{RQ4: Efficiency tradeoffs.} When does selective replay reduce context and latency, and when does multi-stage replay introduce additional wall-clock overhead?
\end{itemize}
...
We use a within-task comparative design over two controlled update tasks in a telehealth policy memo domain. Each task is run under three conditions: a naturalistic loop update, a loop update with explicit edit-event awareness, and execution-lineage DAG replay. All conditions use the same model family and source materials. The comparison is system-level rather than equal-prompt: the DAG condition receives runtime-derived dependency and edit-lineage state because that state is the architectural substrate under evaluation. Loop conditions rely on implicit reconstruction from the prior memo, current materials, and, in one condition, the edit event.
...
The fast demo shows a clear separation between final deliverable correctness and maintained-state correctness. In the unrelated-branch update, DAG replay preserved the final memo exactly while loop baselines rewrote and often contaminated it. In the intermediate-artifact edit, all systems updated the final memo correctly, but only DAG replay maintained perfect upstream preservation, downstream propagation, and cross-artifact consistency. Preservation and propagation are two sides of maintained-state quality: a system must know both when an artifact should remain fixed and when a change should flow downstream.
...
\toprule
Task & Condition & Exact preserve & Churn & Contam. & Constraint reflected & Cross-artifact consistency & Input tokens & Calls & Wall-clock \\
\midrule
Unrelated update & Loop final update & 0.00 & 0.923 & 0.667 & -- & -- & 11655 & 1 & 33.5s \\
Unrelated update & Loop + edit event & 0.00 & 0.908 & 1.000 & -- & -- & 11694 & 1 & 55.0s \\
Unrelated update & DAG replay & 1.00 & 0.000 & 0.000 & -- & -- & 382 & 1 & 7.9s \\
\midrule
Intermediate edit & Loop final update & -- & -- & -- & 1.00 & 0.50 & 11404 & 1 & 38.7s \\
Intermediate edit & Loop + edit event & -- & -- & -- & 1.00 & 0.50 & 11464 & 1 & 31.1s \\
Intermediate edit & DAG replay & -- & -- & -- & 1.00 & 1.00 & 8461 & 2 & 73.6s \\
\bottomrule
\end{tabular}
}
\caption{Fast-demo results ($n=3$ repeats per task-condition). Task-specific maintained-state metrics are shown where applicable; omitted metrics are reported in the text.}
\label{tab:main-results}
\end{table}

\subsection{Dependency Isolation: Unrelated Branch Update}

The unrelated-branch task is the cleanest dependency-isolation case in this study. The correct behavior is not to write a better memo, but to determine that the recruiting update is outside the final memo's dependency path and therefore should not alter it. Under these conditions, DAG replay preserved the final memo exactly in $3/3$ runs, with \texttt{final\_output\_exact\_match} and \texttt{final\_output\_hash\_preserved} both equal to $1.00$. It also achieved \texttt{stable\_artifact\_hash\_preservation} of $1.00$, zero churn, and zero unrelated-branch contamination.

The loop baselines rewrote the final memo in $3/3$ runs. The \texttt{loop\_real\_world\_final\_update} condition contaminated the memo in $2/3$ runs, while the \texttt{loop\_real\_world\_with\_edit\_event} condition contaminated it in $3/3$ runs. This matters because the edit-event loop knew what changed at the source level yet still rewrote the memo and imported unrelated branch content. In this task, source-change awareness alone was insufficient; dependency structure was the missing ingredient.

The efficiency difference was also large on this true unrelated update. DAG replay used $382$ input tokens versus about $11.7$k for either loop baseline, or about $30.5\times$ fewer than \texttt{loop\_real\_world\_final\_update} and $30.6\times$ fewer than \texttt{loop\_real\_world\_with\_edit\_event}. Output tokens were also lower for DAG replay ($440.7$) than for the two loops ($2189.3$ and $2078.3$). It was also about $4.2\times$ faster than the final-update loop ($33.5$s vs.\ $7.9$s) and about $6.9\times$ faster than the edit-event loop ($55.0$s vs.\ $7.9$s). Faithfulness and current-state precision followed the same pattern: DAG replay scored $1.0$ on both, while the loop baselines scored $0.0$ faithfulness with current-state precision of $0.333$ and $0.0$, respectively. In this task, DAG replay was both more correct and more efficient.

\subsection{Artifact-Level Propagation: Intermediate Criteria Edit}

The intermediate-artifact task is the positive propagation case and is at least as important conceptually as the unrelated-update case. Here the correct behavior is not preservation alone. The system must propagate a meaningful edit through downstream artifacts while preserving upstream evidence and unrelated state.

All three conditions reflected the new constraint in the final memo, with \texttt{final\_memo\_constraint\_reflection} equal to $1.00$ in every case. All three also achieved \texttt{output\_faithfulness\_score} and \texttt{current\_state\_precision\_score} of $1.00$. A final-memo-only evaluation would therefore treat all systems as successful. The maintained-state metrics show the real difference.

DAG replay achieved perfect \texttt{stable\_artifact\_hash\_preservation} ($1.00$), \texttt{downstream\_propagation\_recall} ($1.00$), \texttt{upstream\_churn\_rate} ($0.00$), \texttt{unaffected\_artifact\_preservation} ($1.00$), and \texttt{cross\_artifact\_consistency\_score} ($1.00$). The loop baselines updated the visible deliverable; DAG replay updated the work system. Both loop baselines reflected the new constraint in the final memo, but each reached only $0.50$ cross-artifact consistency because the updated final deliverable was not consistently aligned with maintained intermediate state. All three conditions had \texttt{final\_output\_exact\_match} and \texttt{final\_output\_hash\_preserved} equal to $0.0$, which is expected here because the final memo should change.

This task also shows why the paper should not claim general wall-clock superiority. DAG replay used fewer input tokens than the loops ($8461$ versus $11404$ and $11464$, or about $1.35\times$ and $1.36\times$ less input context), but it produced more output tokens ($4311.3$ versus $2418.7$ and $1981.0$) and was slower in wall-clock time because the implementation recomputed downstream artifacts sequentially in two model calls. It took $73.6$s versus $38.7$s for \texttt{loop\_real\_world\_final\_update} and $31.1$s for \texttt{loop\_real\_world\_with\_edit\_event}, making it about $1.9\times$ and $2.36\times$ slower, respectively.
...
The fast-demo results do not support a blanket claim that DAG replay is always faster. On the unrelated-branch update, DAG replay was both more correct and more efficient in tokens and wall-clock time because it reused preserved state and scoped recomputation tightly. On the intermediate edit, DAG replay used less input context but was slower in wall-clock time due to sequential multi-stage replay. The implication is narrower: execution lineage improves maintained-state quality under change, while efficiency depends on the structure of the update and the runtime's replay implementation.
...
The empirical evaluation is small: two tasks, one domain, three conditions, three repeats, and one model family. The tasks were designed to expose DAG-native properties rather than to benchmark all agentic workflows. The loop baselines are not exhaustive; stronger custom loop harnesses with explicit state tracking could narrow some gaps. The DAG implementation is sequential, so wall-clock time does not always improve even when input context and recomputation are reduced. The graph must be correctly specified; dependency mistakes, poor source routing, or lossy artifact interfaces can reduce the advantage. Finally, the results should not be read as evidence that DAG replay generally produces better prose than a holistic loop rewrite.
...
The fast-demo tasks show both sides of execution lineage. In the unrelated-branch update, DAG replay preserved stable work exactly and isolated an adjacent but non-dependent branch. In the intermediate-artifact edit, DAG replay propagated a meaningful change through downstream artifacts while preserving upstream state. Loop baselines were able to update visible final memos, but they were less reliable at preserving stable outputs and maintaining cross-artifact consistency.
